                                                  ...
                                                  size_t *base_length)
{
    ...
    if (length <= sizeof(prefix) - 1U || length > (size_t)PD_RELAY_BASE_URL_MAX_BYTES ||
        memcmp(base, prefix, sizeof(prefix) - 1U) != 0) {
...
static pd_relay_endpoint_result pd_build_https_path(const char *base,
                                                    const char *path,
                                                    char *output,
                                                    size_t output_capacity)
{
    ...
    required = base_length + path_length + 1U;
    ...
    memcpy(&output[base_length], path, path_length + 1U);
    return PD_RELAY_ENDPOINT_OK;
}
...
pd_relay_endpoint_result pd_relay_build_registration_url(const char *https_base_url,
                                                         ...
                                                         size_t output_capacity)
{
    return pd_build_https_path(https_base_url, "/v1/sessions", output, output_capacity);
}
