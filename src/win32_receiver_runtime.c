    ...
    void *event_context)
{
    ...
    char registration_error[PD_RELAY_ERROR_CAPACITY];
    pd_win32_relay_client relay;
    ...
    pd_win32_relay_registration_result registration_result;
    bool curl_initialized = false;
    pd_win32_receiver_runtime_result result = PD_WIN32_RECEIVER_RUNTIME_RELAY_ERROR;
    ...
    if (pd_relay_build_receiver_wss_url(relay_https_base_url,
                                        session->token,
                                        receiver_url,
                                        sizeof(receiver_url)) != PD_RELAY_ENDPOINT_OK) {
        return PD_WIN32_RECEIVER_RUNTIME_ENDPOINT_ERROR;
    }
    if (!pd_win32_curl_global_init()) {
        return PD_WIN32_RECEIVER_RUNTIME_RELAY_ERROR;
    }
    curl_initialized = true;

    registration_result = pd_win32_relay_register_session(relay_https_base_url,
                                                           session->token,
                                                           session->receiver_secret,
                                                           registration_error,
                                                           sizeof(registration_error));
    if (registration_result != PD_WIN32_RELAY_REGISTRATION_OK) {
        result = PD_WIN32_RECEIVER_RUNTIME_REGISTRATION_ERROR;
        goto cleanup_global;
    }
    ...
    if (pd_win32_relay_client_init(&relay,
        ...
        result = PD_WIN32_RECEIVER_RUNTIME_RELAY_ERROR;
    ...
    if (pd_win32_receive_handler_init(&handler, jobs_root_directory) !=
        ...
        pd_win32_relay_client_cleanup(&relay);
        result = PD_WIN32_RECEIVER_RUNTIME_STORAGE_ERROR;
        goto cleanup_session;
    }
    if (pd_receiver_protocol_init(&protocol,
        ...
        pd_win32_relay_client_cleanup(&relay);
        result = PD_WIN32_RECEIVER_RUNTIME_PROTOCOL_ERROR;
        goto cleanup_session;
    }
    ...
    if (loop_result == PD_RECEIVER_LOOP_OK) {
        result = PD_WIN32_RECEIVER_RUNTIME_OK;
    } else if (loop_result == PD_RECEIVER_LOOP_RECEIVE_ERROR ||
